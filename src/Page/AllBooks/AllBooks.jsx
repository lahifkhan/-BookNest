...
const AllBooks = () => {
  ...
  const { data: books = [], isPending } = useQuery({
    queryKey: ["books", search, sortValue],
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/books?status=published&search=${search}&sort=${sortValue}`
      );
      return res.data;
    },
  });
    ...
    <div className="w-11/12 mx-auto my-8">
      <div className="text-center mb-10">
        ...
        <h2 className="text-3xl md:text-4xl font-extrabold text-primary mt-2">
          Discover All Published Books
        </h2>

        <p className="text-accent mt-2 max-w-2xl mx-auto">
          Browse thousands of books added by our librarians. Search, filter, and
